...
void initData(RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
	...
	/*CHANGE twostepflag to false *//*no any more!!!*/
...
/* Port configuration data set */
	...
	//前面是logAnnounceInterval，后面是announceInterval，之后会处理
 ...
 * M1: The port is in the MASTER state because it is on a clockClass 1 through 127 node and is
 * the grandmaster port of the system
 * */
...
//	ptpClock->timeSource = INTERNAL_OSCILLATOR;
	ptpClock->timeSource = GPS;

}


/* Local clock is synchronized to Ebest(An announce message) Table 16 (9.3.5) of the spec
		 ...
		 RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
	...
	if (ptpClock->clockQuality.clockClass < 128) { //时钟等级为0-127
		...
		if ((bmcDataSetComparison(&ptpClock->msgTmpHeader,
					  &ptpClock->msgTmp.announce,
					  header,announce,ptpClock))<0) {
			//M2状态，代码中M1，M2，M3不区分吗？
			m1(ptpClock);
			return PTP_MASTER;
		} else if ((bmcDataSetComparison(&ptpClock->msgTmpHeader,
						 &ptpClock->msgTmp.announce,
						 header,announce,ptpClock)>0)) {
			//这里就直接从时钟状态了，和Fig 26好像有点不同
			//除非说Ebest由当前端口接收
			s1(header,announce,ptpClock);
			return PTP_SLAVE;
		} else {
...
/**
 * 通过数据集比较得到foreign_records的最佳数据集Erbest
 */
UInteger8 
