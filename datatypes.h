...
typedef struct {
	...
	//CHANGE make more space for ethernet header;
	Octet msgIbuf[PACKET_SIZE+MAC_HEADER_LEN];
//	Octet *pObuf;
	Octet *pIbuf;
	...
	/*CHANGE  add this to driver_time got from drivers*/
	TimeInternal 	sync_send_driver_time;
	TimeInternal	pdelay_resp_send_driver_time;
	...
	/*CHANGE add os_sync_receive_time and os delay_req_send_time to correct the carry bit*/
	TimeInternal os_sync_receive_time;
	TimeInternal os_delay_req_send_time;
	...
	/*CHANGE below needed by driver rate adjustment*/
	TimeInternal last_sync_receive_time;
	int32_t currentRate;
	int32_t averagePeriod;
