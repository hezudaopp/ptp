...
toState(UInteger8 state, RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
	...
	case PTP_LISTENING:
		...
		//ptpClock->announceReceiptTimeout的值为6个AnnounceInterval时间间隙
...
doState(RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
	...
	case PTP_MASTER:
		 ...
		 * 决定时钟状态
		 * */
		if(ptpClock->record_update) //发现记录升级事件（比如新添加了时钟）
		{
			DBGV("event STATE_DECISION_EVENT\n");
			ptpClock->record_update = FALSE;
			state = bmc(ptpClock->foreign, rtOpts, ptpClock);//运行BMC算法决定时钟状态
			if(state != ptpClock->portState)
				toState(state, rtOpts, ptpClock);//状态需要改变
		}
		break;
	...
	case PTP_SLAVE:
		...
		//ANNOUNCE_RECEIPT报文过期后会判断是否将时钟改成主时钟或监听状态，具体见Figure 23 State machine for a full implementation
		if(timerExpired(ANNOUNCE_RECEIPT_TIMER, ptpClock->itimer))  
		{
			DBGV("event ANNOUNCE_RECEIPT_TIMEOUT_EXPIRES\n");
			ptpClock->number_foreign_records = 0;
			ptpClock->foreign_record_i = 0;
			//判断是否为主时钟主要是看ptpClock->clockQuality.clockClass的值
			if(!ptpClock->slaveOnly && 
			   ptpClock->clockQuality.clockClass != 255) {
				m1(ptpClock);//将ptpClock设置为主时钟
				toState(PTP_MASTER, rtOpts, ptpClock);//转变为主时钟状态
			} else if(ptpClock->portState != PTP_LISTENING)
				toState(PTP_LISTENING, rtOpts, ptpClock);//转变为监听状态
		}
		
		//E2E模式和P2P模式发送的包不同，DelayReq或PDelayReq包是由从时钟发出的
		if (rtOpts->E2E_mode) {
	...
	case PTP_MASTER:
		...
		//为什么是issuePDelayReq，不是issuePDelayResp
		if (!rtOpts->E2E_mode) {
			if(timerExpired(PDELAYREQ_INTERVAL_TIMER,
					ptpClock->itimer)) {
				DBGV("event PDELAYREQ_INTERVAL_TIMEOUT_EXPIRES\n");
...
handle(RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
	...
	//doState函数ptpClock->message_activity为False,
	if(!ptpClock->message_activity)	{
	...
	msgUnpackHeader(ptpClock->pIbuf, &ptpClock->msgTmpHeader);//头部信息解包
	       ...
	       Boolean isFromSelf, RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
	...
	case PTP_SLAVE:
		 ...
		 * will be executed 
		 */
		ptpClock->record_update = TRUE; //record_update为True执行BMC算法
		...
		case FALSE:
	   		...
	   		addForeign(ptpClock->pIbuf,header,ptpClock);//addForeign会判断是否Foreign Master is already in ForeignMaster data set
...
*/
			...
			if ((header->flagField[0] & 0x02) == TWO_STEP_FLAG) {//双步模式
				...
				updateClock(rtOpts,ptpClock);//同步本地时钟，双步模式的话在接收到FollowUp报文之后处理
	...
	default :
		if (!isFromSelf) {
			...
			/*readCacheTime(time);*/
			*time = ptpClock->sync_send_driver_time;//单步模式记录sync包发送时间
			/*Add latency dont need here*/
//			addTime(time,time,&rtOpts->outboundLatency);
			issueFollowup(time,rtOpts,ptpClock);//单步模式的话还需要再发送followup报文，主要是由slave端决定是否对followup报文进行处理
			break;
		}	
	}
}
	       ...
	       Boolean isFromSelf, RunTimeOpts *rtOpts, PtpClock *ptpClock)
{
	...
	case PTP_SLAVE:
		...
		if (isFromCurrentParent) {
			if (ptpClock->waitingForFollow)	{//如果是单步模式，waitingForFollow为false
