...
//CHANGE ADD timeInternal_readable()
void
timeInternal_readable(TimeInternal * timeInternal, char * msgType)
{
	//fprintf(stderr,"%s has been sent in: %s",msgType,asctime(gmtime((time_t *)&(timeInternal->seconds))));
	DBGV("%s has been sent in: %s",msgType,asctime(gmtime((time_t *)&(timeInternal->seconds))));
...
/*CHANGE display the t2 too*/
void 
msgSync_display(MsgSync * sync, TimeInternal * sync_receive_time)
{
	DBGV("Message Sync : \n");
	DBGV("t1 : \n");
	timestamp_display(&sync->originTimestamp);
	DBGV("t2 : \n");
	timeInternal_display(sync_receive_time);
	DBGV("\n");
}
/*CHANGE ENDS*/
...
void 
msgDelayResp_display(MsgDelayResp * resp, TimeInternal *delay_req_send_time)
{
	DBGV("t3: \n");
	timeInternal_display(delay_req_send_time);
	DBGV("t4: \n");
