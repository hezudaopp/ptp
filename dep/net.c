...
netInit(NetPath * netPath, RunTimeOpts * rtOpts, PtpClock * ptpClock)
{
	if(!rtOpts->ethernet_mode)
	{
		 ...
		 * 设置netPath->unicastAddr
		 * */
		if (rtOpts->unicastAddress[0]) {
			/* Attempt a DNS lookup first. */
			struct hostent *host;
			host = gethostbyname2(rtOpts->unicastAddress, AF_INET);//通过hostname获取hostent，该结构体的h_addr_list包含ip地址信息
					if (host != NULL) {
				if (host->h_length != 4) {
				...
				//如果rtOpts->unicastAddress是十进制的点分地址格式，直接将其转换成二进制地址并赋值给netAddr和netPath->unicastAddr
		...
		imr.imr_multiaddr.s_addr = netAddr.s_addr;//广播地址
		imr.imr_interface.s_addr = interfaceAddr.s_addr;//网卡地址
		//设置socket的IP协议的选项IP_MULTICAST_IF，值为网卡地址
		...
		//设置socket的IP协议的选项IP_ADD_MEMBERSHIP值为imr
		 ...
		 * 除了PEER_PTP_DOMAIN_ADDRESS=224.0.0.107与之前的初始化操作不同，其他类似
		 * */
...
netSelect(TimeInternal * timeout, NetPath * netPath)
{
	...
	FD_ZERO(&readfds);	//设置readfds的所有位为0，声明一个fd_set之后必须的操作
	//构造readfds描述符列表
...
netSendEvent(Octet * buf, UInteger16 length, NetPath * netPath)
{
	...
	if (netPath->unicastAddr) {
		...
		//netPath->multicastAddr为默认多播地址，peer-to-peer和E2E的时候各不相同
