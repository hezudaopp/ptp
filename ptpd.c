...
main(int argc, char **argv)
{
	...
	if (!(ptpClock = ptpdStartup(argc, argv, &ret, &rtOpts)))
		...
		/*Change display runtimeoption*/
		displayRunTimeOpts(&rtOpts);
		/*Change ends*/
        if (rtOpts.probe) {
                // not yet implemented，rtOpts.probe参数做什么用？
        }
        else {
        	/*CHANGE set the time of dp83640*/
/*            TimeInternal time;
            getTime(&time);
            setDriverTime(&time);
            DBGV("set init time to:\n");
            timeInternal_display(&time);
            DBGV("\n");
*/
        	/*CHANGE ENDS*/
		...
		//CHANGE open driver fd here
		//以下语句执行出错， ptpClock->fd < 0
		ptpClock->fd=open(DEVICE_P_FILENAME,O_RDWR|O_NDELAY);
		if(ptpClock->fd < 0) ERROR("cant open driver in ReadCachedTime\n");
		//CHANGE ENDS
		protocol(&rtOpts, ptpClock);
		/* forever loop.. */
	}
        if(ptpClock->fd > 0) close(ptpClock->fd);
        ptpdShutdown();
